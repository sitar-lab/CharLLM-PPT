...
class NVIDIAGPU(gpu_common.GPU):
    ...
    def getAveragePowerUsage(self) -> int:
        ...
        if (ret := metric.nvmlReturn) != pynvml.NVML_SUCCESS:
            # CharLLM-PPT: original code
            # raise pynvml.NVMLError(ret)

            # CharLLM-PPT: record -1 if error occurs
            return -1
    ...
    def getGPUUtilization(self) -> int:
        """Return the current power draw of the GPU. Units: mW."""
        metric = pynvml.nvmlDeviceGetUtilizationRates(self.handle)
    ...
    def getTemperature(self) -> int:
        ...
        metric = pynvml.nvmlDeviceGetTemperature(
    ...
    def getMemTemperature(self) -> int:
        """Return the current temperature of the GPU. Units: Degree C."""
        metric = pynvml.nvmlDeviceGetFieldValues(
            self.handle, [pynvml.NVML_FI_DEV_MEMORY_TEMP]
        )[0]
        if (ret := metric.nvmlReturn) != pynvml.NVML_SUCCESS:
            # CharLLM-PPT: original code
            # raise pynvml.NVMLError(ret)
    ...
    def getSMClock(self) -> int:
        """Return the clock speed of the SM in MHz."""
        metric = pynvml.nvmlDeviceGetClockInfo(self.handle, pynvml.NVML_CLOCK_SM)
    ...
    def getMemoryClock(self) -> int:
        """Return the clock speed of the Memory in MHz."""
        metric = pynvml.nvmlDeviceGetClockInfo(self.handle, pynvml.NVML_CLOCK_MEM)
    ...
    def getInstantMemoryPowerUsage(self) -> int:
        """Return the instant power draw of the GPU's memory. Units: mW.
        ...
        if power == 0:
            warnings.warn(
                "Instant memory power returned 0. The current GPU may not be supported.",
                stacklevel=1,
            )
        return power
    ...
    def getPCIeTxThroughput(self) -> int:
        """Return the PCIe throughput of the GPU. Units: MB/s."""
        metric = pynvml.nvmlDeviceGetPcieThroughput(
    ...
    def getPCIeTxBytes(self) -> int:
        """Return the PCIe throughput of the GPU. Units: MB/s."""
        metric = pynvml.nvmlDeviceGetFieldValues(
            self.handle, [pynvml.NVML_FI_DEV_PCIE_COUNT_TX_BYTES]
        )[0]
        if (ret := metric.nvmlReturn) != pynvml.NVML_SUCCESS:
            # CharLLM-PPT: original code
            # raise pynvml.NVMLError(ret)
    ...
    def getPCIeRxBytes(self) -> int:
        """Return the PCIe throughput of the GPU. Units: MB/s."""
        metric = pynvml.nvmlDeviceGetFieldValues(
            self.handle, [pynvml.NVML_FI_DEV_PCIE_COUNT_RX_BYTES]
        )[0]
    ...
    def getNVLinkTxThroughput(self) -> int:
        """Return the NVLink throughput of the GPU. Units: MB/s."""
        metric = pynvml.nvmlDeviceGetFieldValues(
            self.handle, [pynvml.NVML_FI_DEV_NVLINK_THROUGHPUT_RAW_TX]
        )[0]
    ...
    def getNVLinkTxBytes(self) -> int:
        """Return the NVLink throughput of the GPU. Units: MB/s."""
        metric = pynvml.nvmlDeviceGetFieldValues(
            self.handle, [pynvml.NVML_FI_DEV_NVLINK_COUNT_XMIT_BYTES]
        )[0]
    ...
    def getNVLinkRxThroughput(self) -> int:
        """Return the NVLink throughput of the GPU. Units: MB/s."""
        metric = pynvml.nvmlDeviceGetFieldValues(
            self.handle, [pynvml.NVML_FI_DEV_NVLINK_THROUGHPUT_RAW_RX]
        )[0]
    ...
    def getNVLinkRxBytes(self) -> int:
        """Return the NVLink throughput of the GPU. Units: MB/s."""
        metric = pynvml.nvmlDeviceGetFieldValues(
            self.handle, [pynvml.NVML_FI_DEV_NVLINK_COUNT_RCV_BYTES]
        )[0]

...
class AMDGPU(gpu_common.GPU):
    ...
    def getGPUUtilization(self) -> int:
        """Return the compute utilization of the GPU. Units: %."""
        engine_usage = amdsmi.amdsmi_get_gpu_activity(self.handle)
        metric = engine_usage["gfx_activity"]
    ...
    def getMemoryUtilization(self) -> int:
        """Return the memory usage of the GPU. Units: %."""
        vram_memory_total = amdsmi.amdsmi_get_gpu_memory_total(
            self.handle, amdsmi.amdsmi_interface.AmdSmiMemoryType.VRAM
        )
        vram_memory_usage = amdsmi.amdsmi_get_gpu_memory_usage(
            self.handle, amdsmi.amdsmi_interface.AmdSmiMemoryType.VRAM
        )
        metric = (vram_memory_usage / vram_memory_total) * 100
    ...
    def getTemperature(self) -> int:
        ...
        metric = amdsmi.amdsmi_get_temp_metric(
            self.handle,
    ...
    def getMemTemperature(self) -> int:
        """Return the current temperature of the VRAM. Units: Degree C."""
        metric = amdsmi.amdsmi_get_temp_metric(
            self.handle,
    ...
    def getPCIeTxThroughput(self) -> int:
        ...
        throughput = amdsmi.amdsmi_get_gpu_pci_throughput(self.handle)
        metric = throughput["sent"] / 1024  # convert to MB/s
    ...
    def getPCIeRxThroughput(self) -> int:
        """Return the PCIe throughput of the GPU. Units: MB/s."""
        throughput = amdsmi.amdsmi_get_gpu_pci_throughput(self.handle)
        metric = throughput["received"] / 1024  # convert to MB/s

        return metric

    # Note: NVLink is not supported on AMD GPUs, returning 0 for both RX and TX throughput.
    @_handle_amdsmi_errors
    ...
    def getAveragePowerUsage(self) -> int:
        ...
        return int(amdsmi.amdsmi_get_power_info(self.handle)["average_socket_power"])
...
class AMDGPUs(gpu_common.GPUs):
    ...
    def _init_gpus(self) -> None:
        ...
        # Always measure power and not energy
        for gpu in self._gpus:

...
class ModelPT(LightningModule, Model):
    ...
    def on_train_start(self):
        ...
        # CharLLM-PPT: Power analysis
        if os.environ.get("ZEUS_MONITOR_ENABLED", "0") == "1":
            num_gpus = torch.cuda.device_count()
            global_rank = get_rank()
            local_rank = global_rank % num_gpus
            
            print(f"Initializing monitor for global rank {global_rank} and local rank {local_rank}")
            self._monitor = ZeusMonitor(global_rank=global_rank, gpu_indices=[local_rank], sync_execution_with="torch")
            
            # # Start monitoring from the very first batch
            ...
            if hasattr(self, '_chakra_profile_enabled'):
                if self._chakra_profile_enabled and not self._chakra_profile_in_progress:
                    if (
                        ...
                        if os.environ.get("ZEUS_MONITOR_ENABLED", "0") == "1":
                            # Start monitoring from the very first batch
                            print("Starting monitoring on rank", get_rank())
                            self._monitor.begin_window("train")
                        
                        # CharLLM-PPT: Per-Pipeline profiling
        ...
        if self.device.type == 'cuda':
            if hasattr(self, '_chakra_profile_enabled'):
                ...
                if self.trainer.global_step - 1 >= self._chakra_profile_end_step:
                    if os.environ.get("ZEUS_MONITOR_ENABLED", "0") == "1":
                        print(f"DEBUG: Ending monitoring on rank {get_rank()}")
                        self._monitor.end_window("train")
    ...
    def on_test_start(self):
        """PyTorch Lightning hook:
        https://pytorch-lightning.readthedocs.io/en/stable/common/lightning_module.html#on-test-start
        We use it here to copy the relevant config for dynamic freezing.
        """
        # CharLLM-PPT: Power analysis
        if os.environ.get("ZEUS_MONITOR_ENABLED", "0") == "1":
    ...
    # CharLLM-PPT
    def on_test_batch_start(self, batch: Any, batch_idx: int, unused: int = 0) -> Optional[int]:
        """PyTorch Lightning hook:
        https://pytorch-lightning.readthedocs.io/en/stable/common/lightning_module.html#on-test-batch-start
        We use it here to enable profiling.
        """
        
        print(f"DEBUG: on_test_batch_start called with batch_idx {batch_idx}")
        if self.device.type == 'cuda':
            # if hasattr(self, '_chakra_profile_enabled'):
                # if self._chakra_profile_enabled and not self._chakra_profile_in_progress:
            print(f"DEBUG: self._chakra_profile_enabled: {getattr(self, '_chakra_profile_enabled', False)}")
            print(f"DEBUG: self._chakra_profile_in_progress: {self._chakra_profile_in_progress}")
            print(f"DEBUG: self._chakra_profile_start_step: {self._chakra_profile_start_step}")
            print(f"DEBUG: self._chakra_profile_end_step: {self._chakra_profile_end_step}")
            if not self._chakra_profile_in_progress:
                if (
                    batch_idx >= self._chakra_profile_start_step
                    and batch_idx <= self._chakra_profile_end_step
                ):
                    # CharLLM-PPT: Power Analysis
                    if os.environ.get("ZEUS_MONITOR_ENABLED", "0") == "1":
                        # Start monitoring from the very first batch
                    ...
                    if parallel_state.get_tensor_model_parallel_rank() == 0 and parallel_state.get_data_parallel_rank() == 0:
                        logging.info(
                            f"====== Start chakra profiling from batch_idx {batch_idx} ======"
                        )
    ...
    # CharLLM-PPT
    def on_test_batch_end(self, outputs, batch: Any, batch_idx: int, unused: int = 0) -> None:
        """PyTorch Lightning hook:
        https://pytorch-lightning.readthedocs.io/en/stable/common/lightning_module.html#on-test-batch-end
        We use it here to enable nsys profiling.
        """
        print(f"DEBUG: on_test_batch_end called with batch_idx {batch_idx}")
        if self.device.type == 'cuda':
            # if hasattr(self, '_chakra_profile_enabled'):
            # CharLLM-PPT: Power analysis
            if batch_idx >= self._chakra_profile_end_step:
                if os.environ.get("ZEUS_MONITOR_ENABLED", "0") == "1":
                    print(f"DEBUG: Ending monitoring on rank {get_rank()}")
                    self._monitor.end_window("train")
            ...
            # self.trainer.global_step is increaeasd before on_train_batch_end
            # if self._chakra_profile_enabled and self._chakra_profile_in_progress:
            if self._chakra_profile_in_progress:
                if batch_idx >= self._chakra_profile_end_step:
                    
                    # CharLLM-PPT: Per-Pipeline profiling
                    if parallel_state.get_tensor_model_parallel_rank() == 0 and parallel_state.get_data_parallel_rank() == 0:
                        logging.info(f"====== End chakra profiling at batch_idx {batch_idx} ======")
                ...
                elif batch_idx >= self._chakra_profile_start_step:
                    self._prof.step()

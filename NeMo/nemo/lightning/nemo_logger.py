    ...
    def setup(self, trainer: Union[pl.Trainer, fl.Fabric], resume_if_exists: bool = False, task_config=None):
        ...
        if self.explicit_log_dir and isinstance(trainer, pl.Trainer):
            ...
            if self.log_dir is None:
                # Seokjin: If we are using distributed exp, we should use the rank to create the exp_dir.
                rank = trainer.global_rank if isinstance(trainer, pl.Trainer) else trainer.global_rank
                _dir = str(Path.cwd() / f"nemo_experiments_{rank}")

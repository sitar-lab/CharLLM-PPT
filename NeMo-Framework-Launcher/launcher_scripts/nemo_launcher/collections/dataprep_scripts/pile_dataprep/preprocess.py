...
def main(cfg):
    ...
    # megatron_dir = cfg.get("megatron_dir")
    
    # CharLLM-PPT: import envvar CHARLLM_ROOT
    charllm_root = os.environ.get("CHARLLM_ROOT", None)
    
    # CharLLM-PPT: if CHARLLM_ROOT is not set, exit with an error
    if not charllm_root:
        raise EnvironmentError("CHARLLM_ROOT environment variable is not set.")
    
    # CharLLM-PPT: change this part.
    # megatron_dir = "/home/hice1/sgo38/scratch/chakra/CharLLM-PPT/NeMo/nemo/collections/nlp/data/language_modeling/megatron"
    megatron_dir = os.path.join(
        charllm_root,
        "CharLLM-PPT",
        "NeMo",
        "nemo",
    ...
    # CharLLM-PPT: change this part.
    # code_path = (
    #     "/home/hice1/sgo38/scratch/chakra/CharLLM-PPT/NeMo/scripts/nlp_language_modeling/preprocess_data_for_megatron.py"
    # )
    ...
    hf_cache = os.environ.get(
        "TRANSFORMERS_CACHE", os.environ.get("HF_HOME", None)
    )
    # CharLLM-PPT: change this part.
    runcmd = (
        f"cd {megatron_dir}; "
        f'export PYTHONPATH="{charllm_root}/CharLLM-PPT/NeMo/.:$PYTHONPATH"; '

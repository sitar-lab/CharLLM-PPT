...
def create_rank_plots(base_dir: str, output_dir: str, target_model: str, crop_mode: str = 'full'):
    ...
    # Second pass to plot each config's data
    for config, config_idx in model_configs.items():
        csv_path = os.path.join(base_dir, target_model, f"{target_model}_{config_idx}_mbs1_kernel_stats.csv")
        if os.path.isfile(csv_path):
            kernel_data = get_kernel_data(csv_path, config)
            if kernel_data:
                ...
                global_max_time = 40 # Set manually for consistent y-axis scaling
...
def main():
    ...
    output_dir = "${CHARLLM_ROOT}/CharLLM-PPT/figures/microbatch-sweep-h200-mbs1/"

...
def get_model_sort_key(model_name: str) -> tuple:
    """Get sort key for model name and config to ensure correct ordering"""
    # Split into model and config (e.g., 'M8x7_T2P8' -> ['M8x7', 'T2P8'])
    parts = model_name.split('_')
    model_prefix = parts[0]
    config = parts[1] if len(parts) > 1 else ''
    
    # Get TP degree from config
    tp_match = re.search(r'T(\d+)', config)
    tp_degree = int(tp_match.group(1)) if tp_match else 0
    
    # Get PP degree from config
    pp_match = re.search(r'P(\d+)', config)
    pp_degree = int(pp_match.group(1)) if pp_match else 0
    
    # Handle different model types
    if model_prefix.startswith('G'):  # GPT
        size = int(model_prefix[1:])
        return (1, size, -tp_degree, pp_degree)  # Sort TP descending
    elif model_prefix.startswith('L'):  # LLaMA
        size = int(model_prefix[1:])
        return (2, size, -tp_degree, pp_degree)  # Sort TP descending
    elif model_prefix.startswith('M'):  # Mixtral
        size = int(model_prefix[3:] if 'x' in model_prefix else model_prefix[1:])
        return (3, size, -tp_degree, pp_degree)  # Sort TP descending
    
    return (0, 0, -tp_degree, pp_degree)  # Default case
...
def create_heatmap(data, title, output_path, cmap='YlOrRd', fmt='.1f', execution_times=None, peak=False):
    ...
    if any(metric in title.lower() for metric in ['nvlink', 'pcie', 'ib']):
        ...
        if vmin <= 0:
            vmin = data[data > 0].min().min() / 10  # Set minimum to smallest non-zero value/10
    ...
    ax_main.set_ylabel('Configuration', fontsize=12)
...
def main():
    ...
    figure_base_dir = "${CHARLLM_ROOT}/CharLLM-PPT/figures/heatmaps"
    ...
    for metric_type, subtypes in metrics.items():
        for subtype, label in subtypes.items():
            try:
                ...
                # Sort by custom ordering: descending TP within each model
                df_avg = df_avg.reindex(sorted(df_avg.index, key=get_model_sort_key))
                df_peak = df_peak.reindex(sorted(df_peak.index, key=get_model_sort_key))

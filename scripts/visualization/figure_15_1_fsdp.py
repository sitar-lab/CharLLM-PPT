...
def get_kernel_data(exp_path: str, config: str) -> Dict[str, List[float]]:
    ...
    try:
        ...
        # Get individual rank durations (even indices only)
...
def create_rank_plots(base_dir: str, output_dir: str, target_model: str, crop_mode: str = 'full'):
    ...
    for config, config_idx in model_configs.items():
        csv_path = os.path.join(base_dir, target_model, f"{target_model}_{config_idx}_mbs1_kernel_stats.csv")
    ...
    # Add "Overlap" item to legend with color 'Overlap': '#ff9da7',             # Pink
...
def main():
    ...
    output_dir = "${CHARLLM_ROOT}/CharLLM-PPT/figures/microbatch-sweep-h200-fsdp-mbs1/"

...
def format_model_name(model: str) -> str:
    ...
    if model.startswith('gpt'):
        return model.replace('gpt3', 'GPT ').replace('-175b', '').replace('-fsdp', '')
    elif model.startswith('llama'):
        return model.replace('llama3', 'Llama').replace('-70b', '').replace('-fsdp', '')
...
def create_kernel_barchart(base_dir_1: str, base_dir_2: str, output_dir: str, crop_mode: str) -> None:
    ...
    # Get configs based on directory names
    ...
    current_pos = 0
    bar_positions_1 = []  # Positions for first set of bars
    bar_positions_2 = []  # Positions for second set of bars
    bar_labels = []
    ...
    for model in model_configs:
        ...
        # Adjust mid_point based on model type (considering text length)
        if 'gpt' in model:
            mid_point -= 0.7
        elif 'llama' in model:
            mid_point -= 0.7
    ...
    ax.set_ylim(0, 1800) # Set manually
    ...
    ax.yaxis.set_label_coords(-0.5, 0.5)  # Adjust position
        
    # Set white image background
    fig.patch.set_facecolor('white')  # Set figure background to white
    
    # Save plot with different cropping options
    if crop_mode == 'full':
        # Save complete plot
        output_file = os.path.join(output_dir, f"model_parallel_kernel_times_{crop_mode}-fsdp.pdf")
...
def main():
    base_dir_1 = "${CHARLLM_ROOT}/CharLLM-PPT/results/kineto-optimization-sweep-h200-fsdp"
    base_dir_2 = "${CHARLLM_ROOT}/CharLLM-PPT/results/kineto-optimization-sweep-h200-fsdp"
    output_dir = "${CHARLLM_ROOT}/CharLLM-PPT/figures/figures-2cols-fsdp"

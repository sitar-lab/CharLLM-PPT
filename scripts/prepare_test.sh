...
# Step 1: Go through all the files under ${CHARLLM_ROOT}/CharLLM-PPT/scripts/test then replace the following words:
...
find "${CHARLLM_ROOT}/CharLLM-PPT/scripts/test" -type f \( -name "*.sbatch" -o -name "*.yaml" \) -print0 | while IFS= read -r -d '' file; do
...
# Step 2: Go through all the files under ${CHARLLM_ROOT}/CharLLM-PPT/scripts/test then replace all SBATCH lines (except --error and --output)
echo "Step 2: Replacing SBATCH lines in scripts..."

find "${CHARLLM_ROOT}/CharLLM-PPT/scripts/test" -type f \( -name "*.sbatch" -o -name "*.py" \) -print0 | while IFS= read -r -d '' file; do
        ...
        while IFS= read -r line; do
            ...
            if [[ "$line" == "#SBATCH"* ]]; then
                ...
                if [[ "$line" == *"module load anaconda3"* ]]; then
                    ...
                    if [ -n "${NUM_NODES}" ]; then
                        if [[ "$(basename "$file")" == *"dataprep"* ]]; then
                            echo "#SBATCH --nodes=1" >> "$temp_file"
                        else
                            echo "#SBATCH --nodes=${NUM_NODES}" >> "$temp_file"
                        fi
                    fi
                    if [ -n "${NTASKS_PER_NODE}" ] && [[ "$(basename "$file")" != *"dataprep"* ]]; then
                        echo "#SBATCH --ntasks-per-node=${NTASKS_PER_NODE}" >> "$temp_file"
                    fi
                    if [ -n "${CPUS_PER_TASK}" ] && [[ "$(basename "$file")" != *"dataprep"* ]]; then

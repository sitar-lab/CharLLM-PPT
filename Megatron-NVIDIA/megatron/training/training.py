...
def post_training_step_callbacks(model, optimizer, opt_param_scheduler, iteration, prof,
    ...
    if args.profile and \
        ...
        if args.use_pytorch_profiler:
            ...
            # CharLLM-PPT: Stop the monitor
            if os.environ.get("ZEUS_MONITOR_ENABLED", "0") == "1":
                global monitor
                if monitor is not None:
                    monitor.end_window("train")
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    # CharLLM-PPT: Initialize the monitor    
    if os.environ.get("ZEUS_MONITOR_ENABLED", "0") == "1":
        global_rank = torch.distributed.get_rank()
        gpu_indices = [torch.distributed.get_rank() % torch.cuda.device_count()]
                
        global monitor
        monitor = ZeusMonitor(global_rank=global_rank, gpu_indices=gpu_indices, sync_execution_with="torch")

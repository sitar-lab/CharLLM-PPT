...
def get_transformer_layer_offset(config: TransformerConfig):
    ...
    # CharLLM-PPT: Custom logic for uneven pipeline parallelism
    if os.getenv("CHARLLM_UNEVEN_PIPELINE", "0") == "1":
        num_ranks = config.pipeline_model_parallel_size
        num_layers = config.num_layers

        # Only apply custom logic if pipeline parallelism is enabled and no uneven pipeline config
        if num_ranks > 1 and (
            config.num_layers_in_first_pipeline_stage is None and
            config.num_layers_in_last_pipeline_stage is None
        ):
            # Calculate base and extra layers
            base = num_layers // num_ranks
            remainder = num_layers % num_ranks

            # Build the alternating pattern: e.g. [3,4,3,4,...]
            # Start with base, then base+1, then base, then base+1, etc.
            # Distribute the remainder to the first ranks in the pattern
            layer_counts = []
            ...
            # Adjust for total layers (if sum is not equal, fix the last rank)
            total_assigned = sum(layer_counts)
            if total_assigned < num_layers:
                layer_counts[-1] += num_layers - total_assigned
            elif total_assigned > num_layers:
                layer_counts[-1] -= total_assigned - num_layers

            # Compute offset for this rank
            offset = sum(layer_counts[:pipeline_rank])
            return offset
    
    # CharLLM-PPT: Original logic for even pipeline parallelism

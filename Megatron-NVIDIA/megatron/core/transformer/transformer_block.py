...
def get_num_layers_to_build(config: TransformerConfig) -> int:
    ...
    """
    # CharLLM-PPT: Custom uneven pipeline logic: alternate n-1, n+1 layers
    if os.getenv("CHARLLM_UNEVEN_PIPELINE", "0") == "1":
        ...
        if (
            config.num_layers_in_first_pipeline_stage is None
            and config.num_layers_in_last_pipeline_stage is None
            and num_ranks > 1
        ):
            base = num_layers // num_ranks
            layer_counts = []
            for i in range(num_ranks):
                count = base + 1 if i % 2 == 0 else base - 1
            ...
            return layer_counts[pipeline_rank]
